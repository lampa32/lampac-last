...
mkdir $DEST -p
cd $DEST
if ! curl -L -k -o publish.zip https://github.com/lampa32/lampac/releases/latest/download/publish.zip; then
...
# automatic updates
curl -k -s https://api.github.com/repos/lampa32/lampac/releases/latest | grep tag_name | sed s/[^0-9]//g > $DEST/data/vers.txt
